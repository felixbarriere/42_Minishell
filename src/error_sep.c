...
bool is_type(t_token* current, t_token_type type)
{
	...
	{
		printf("CURRENT 2eme else if = %d\n", type);
		return (true);
	}
	return (false);
}

int	check_error_sep_test(t_token *token_lst)
{
	if ((is_type(token_lst, R_RIGHT) && is_type(token_lst->next, R_LEFT)))
	{
		printf("hola que tal?\nmuy bien y tu?\ncomo te lamas?\n");
		return (FAILURE);
	}
...
int	check_error_sep_1(t_token *token_lst)
{
 ...
 if	((is_type(token_lst, DR_LEFT) && is_type(token_lst->next, R_LEFT)) ||
...
int	check_error_sep_6(t_token *token_lst)
{
	...
	if ((is_type(token_lst->prev, STR) && is_type(token_lst, R_LEFT) 
		...
		|| (is_type(token_lst->prev, STR) && is_type(token_lst, DR_RIGHT)
			&& is_type(token_lst->next, BLANK))
		|| (is_type(token_lst->prev, STR) && is_type(token_lst, PIPE)
			&& is_type(token_lst->next, BLANK))
...
int	check_error_sep(t_token *token_lst)
{
	...
	while(token_lst)
	{
		if (check_error_sep_0(token_lst) == FAILURE)
			...
			return (FAILURE);
		// printf("hello");
		// check_error_sep_test(token_lst);
